...
> 참고: 일부 환경에서 Wikimedia가 403을 반환할 수 있어 `User-Agent` 헤더를 넣었습니다.
...
### 3.1.1 로컬 대신 HF 추론 엔드포인트 사용

Pipeline은 기본적으로 로컬 추론이지만, Hugging Face가 제공하는 **Inference Endpoints**나
**HF Inference (serverless)** API로 원격 추론도 할 수 있습니다.


추가로 추천하는 두 가지 방법 (HF Inference API):

1) `InferenceClient` 사용 (권장: 간단/에러 처리/타임아웃 지원)
```python
import os
from io import BytesIO
...
| 방식 | 장점 | 단점 | 추천 상황 |
|------|------|------|-----------|
| InferenceClient | 코드 간결, 재시도/타임아웃 등 편의 기능 | 라이브러리 의존 | 빠르게 붙일 때, 프로토타입 |
| REST 직접 호출 | 의존성 최소, 동작이 명확 | 에러 처리 직접 구현 | 서비스 통합, 디버깅/로그 |

Inference Endpoints vs HF Inference 한 줄 비교:
- Endpoints = 전용(유료) 인프라/스케일/모니터링, HF Inference = 서버리스/가벼운 호출

참고 문서:
```text
https://huggingface.co/docs/inference-endpoints/index
https://huggingface.co/docs/inference-providers/main/en/providers/hf-inference
```
