...
**Task/Model 정보는 어디서 얻나?**
- Task 이름(파이프라인 식별자): Transformers Pipeline 문서의 task identifier 목록 확인
- Model 이름: Hugging Face Hub 모델 카드의 모델 ID 사용 (`author/model-name`)
```text
...
AutoClass는 **"모델 종류를 몰라도 자동으로 맞는 클래스를 골라주는 어댑터"**입니다.
즉, "이 모델은 BERT인가? GPT인가?"를 직접 고르지 않아도 됩니다.

- 모델 카드/설정(config)에 적힌 정보를 읽고
- 그에 맞는 **토크나이저/모델 클래스**를 알아서 매칭합니다.

#### 왜 필요한가?

모델마다 클래스가 다릅니다.
예: `BertTokenizer`, `GPT2Tokenizer`, `ViTImageProcessor` ...

AutoClass를 쓰면 아래처럼 **모델 이름만 바꿔도 코드가 유지**됩니다.
...
#### AutoClass가 내부적으로 하는 일 (핵심 흐름)

1) Hub에서 `config.json`을 읽음  
2) `model_type`을 확인 (예: "bert", "gpt2", "t5")  
3) 해당하는 실제 클래스에 자동 매핑  
4) 가중치/토크나이저 파일을 다운로드하고 로드

#### 한눈에 보는 흐름 (도식)

```
모델 이름 → config.json → model_type
                 ↓
       (AutoClass가 매핑)
                 ↓
     실제 클래스 선택 (BERT/GPT/ViT)
                 ↓
     가중치 + 토크나이저 로드
```

#### AutoModel vs AutoModelForTask

- `AutoModel`은 **헤드가 없는 기본 모델**  
  (특징 추출/임베딩에 적합)
- `AutoModelForSequenceClassification`처럼  
  **Task용 헤드가 붙은 모델**을 쓰면 바로 학습/추론 가능
