# Part 3.1: Small LLM 선택과 양자화

---

## 학습 목표
- 주요 Small LLM 모델 이해 및 선택 기준
- 양자화(Quantization) 개념과 BitsAndBytes 사용법
- 모델 로딩 최적화 기법
- QLoRA 기반 효율적 파인튜닝

---

## 1. Small Language Model (SLM) 개요

### 1.1 SLM이란?

**Small Language Model (SLM)**: 1B~10B 파라미터 범위의 경량화된 언어 모델

```
LLM (70B+)  →  고성능, 고비용, 서버 필수
SLM (1B-10B) →  효율적, 저비용, 엣지 배포 가능
```

### 1.2 SLM 활용 시나리오

| 시나리오 | 추천 모델 크기 | 이유 |
|---------|--------------|------|
| 모바일/엣지 배포 | 0.5B - 3B | 메모리 제한, 빠른 응답 |
| 개인 GPU (16GB) | 3B - 8B | RTX 3090/4090 |
| 개발/프로토타이핑 | 1B - 7B | 빠른 실험 반복 |
| RAG 파이프라인 | 3B - 8B | 컨텍스트 처리 효율 |
| Function Calling | 7B - 14B | Tool use 능력 |

---

## 2. 주요 Small LLM 모델 비교

### 2.1 모델 패밀리 개요

| 모델 | 개발사 | 크기 범위 | 라이선스 | 특징 |
|------|--------|----------|---------|------|
| **Llama 3.2** | Meta | 1B, 3B | Llama 3.2 License | 범용성, 커뮤니티 지원 |
| **Qwen 3** | Alibaba | 0.6B~32B | Apache 2.0 | 다국어, 긴 컨텍스트, 코딩 |
| **Gemma 3** | Google | 1B, 4B, 12B, 27B | Gemma License | 추론 속도, 안정성 |
| **Kimi K2.5** | Moonshot AI | ~1T (MoE) | Custom | 장문 컨텍스트, 추론/코딩 |
| **Phi-3/4** | Microsoft | 3.8B, 7B, 14B | MIT | 추론/수학, 엣지 최적화 |
| **SmolLM** | HuggingFace | 135M~1.7B | Apache 2.0 | 초경량, 완전 오픈 |
| **Mistral** | Mistral AI | 7B | Apache 2.0 | 성능/효율 균형 |

### 2.2 용도별 추천 모델

```python
# 용도별 추천 모델 (2024-2025 기준)

recommendations = {
    # 범용 대화
    "general_chat": [
        "Qwen/Qwen3-8B",
        "meta-llama/Llama-3.2-3B-Instruct",
        "google/gemma-3-4b-it",
        "moonshotai/Kimi-K2.5-Instruct",
    ],
    
    # 코딩
    "coding": [
        "Qwen/Qwen3-8B",
        "deepseek-ai/deepseek-coder-6.7b-instruct",
        "moonshotai/Kimi-K2.5-Instruct",
        "codellama/CodeLlama-7b-Instruct-hf",
    ],
    
    # 수학/추론
    "reasoning": [
        "microsoft/Phi-3-mini-4k-instruct",
        "Qwen/Qwen3-8B",
        "moonshotai/Kimi-K2.5-Instruct",
        "deepseek-ai/DeepSeek-R1-Distill-Qwen-7B",
    ],
    
    # 다국어
    "multilingual": [
        "Qwen/Qwen3-8B",          # 100+ languages
        "google/gemma-3-4b-it",   # multilingual support
    ],
    
    # 초경량 (엣지/모바일)
    "edge": [
        "HuggingFaceTB/SmolLM2-1.7B-Instruct",
        "Qwen/Qwen3-0.6B",
        "meta-llama/Llama-3.2-1B-Instruct",
    ],
}
```

### 2.3 모델 카드 비교 로드

```python
from transformers import AutoConfig
from huggingface_hub import HfApi

def compare_models(model_ids: list[str]):
    """여러 모델의 설정 비교"""
    api = HfApi()
    
    for model_id in model_ids:
        print(f"\n{'='*50}")
        print(f"Model: {model_id}")
        print('='*50)
        
        # 설정 로드
        config = AutoConfig.from_pretrained(model_id, trust_remote_code=True)
        
        # 기본 정보
        print(f"Hidden size: {config.hidden_size}")
        print(f"Num layers: {config.num_hidden_layers}")
        print(f"Num attention heads: {config.num_attention_heads}")
        print(f"Vocab size: {config.vocab_size}")
        
        # 모델 정보 가져오기
        model_info = api.model_info(model_id)
        print(f"Downloads (last month): {model_info.downloads:,}")
        print(f"Likes: {model_info.likes}")
        print(f"License: {model_info.card_data.license if model_info.card_data else 'N/A'}")

# 비교 실행
compare_models([
    "Qwen/Qwen3-4B",
    "meta-llama/Llama-3.2-3B-Instruct",
    "microsoft/Phi-3-mini-4k-instruct",
])
```

---

## 3. 기본 모델 로딩

### 3.1 AutoModelForCausalLM 기본 사용

```python
from transformers import AutoModelForCausalLM, AutoTokenizer
import torch

model_id = "Qwen/Qwen3-4B"

# 토크나이저 로드
tokenizer = AutoTokenizer.from_pretrained(model_id)

# 모델 로드 (기본: fp32)
model = AutoModelForCausalLM.from_pretrained(
    model_id,
    device_map="auto",  # GPU 자동 배치
    torch_dtype=torch.bfloat16,  # 메모리 절약
)

print(f"Model dtype: {model.dtype}")
print(f"Memory footprint: {model.get_memory_footprint() / 1e9:.2f} GB")
```

### 3.2 Chat Template 적용

```python
# 대화 형식 정의
messages = [
    {"role": "system", "content": "You are a helpful AI assistant."},
    {"role": "user", "content": "Hello! How are you?"}
]

# Chat template 적용
prompt = tokenizer.apply_chat_template(
    messages,
    tokenize=False,
    add_generation_prompt=True
)

print("Formatted prompt:")
print(prompt)

# 토큰화 및 추론
inputs = tokenizer(prompt, return_tensors="pt").to(model.device)

with torch.no_grad():
    outputs = model.generate(
        **inputs,
        max_new_tokens=100,
        do_sample=True,
        temperature=0.7,
        top_p=0.9,
        pad_token_id=tokenizer.eos_token_id
    )

response = tokenizer.decode(outputs[0], skip_special_tokens=True)
print(f"\nResponse:\n{response}")
```

### 3.3 Pipeline 사용 (간편)

```python
from transformers import pipeline
import torch

pipe = pipeline(
    "text-generation",
    model="Qwen/Qwen3-4B",
    torch_dtype=torch.bfloat16,
    device_map="auto",
)

messages = [
    {"role": "user", "content": "Python으로 퀵소트 구현해줘"}
]

output = pipe(messages, max_new_tokens=500)
print(output[0]["generated_text"][-1]["content"])
```

### 3.4 Inference Provider 사용 (모델 다운로드 없이)

Hugging Face의 Inference Provider를 사용하면 로컬에 모델을 저장하지 않고도 원격에서 바로 추론할 수 있습니다.  
아래는 같은 모델을 서로 다른 클라이언트로 호출하는 예시입니다.

- 예시 링크: https://huggingface.co/Qwen/Qwen3-8B?inference_api=true&inference_provider=nscale&language=python&client=openai

#### 3.4.1 OpenAI 호환 클라이언트

```python
from openai import OpenAI

# Hugging Face Inference Provider 엔드포인트를 OpenAI SDK로 사용
client = OpenAI(
    base_url="https://api-inference.huggingface.co/v1",
    api_key="hf_your_token",
)

response = client.chat.completions.create(
    model="Qwen/Qwen3-8B",
    messages=[
        {"role": "user", "content": "한국어로 간단한 자기소개를 해줘."}
    ],
    temperature=0.7,
    max_tokens=200,
)

print(response.choices[0].message.content)
```

#### 3.4.2 `huggingface_hub` 클라이언트

```python
from huggingface_hub import InferenceClient

client = InferenceClient(
    provider="nscale",
    api_key="hf_your_token",
)

response = client.chat.completions.create(
    model="Qwen/Qwen3-8B",
    messages=[
        {"role": "user", "content": "Summarize this in one sentence."}
    ],
    max_tokens=120,
)

print(response.choices[0].message.content)
```

#### 3.4.3 `requests`로 직접 호출

```python
import requests

api_url = "https://api-inference.huggingface.co/v1/chat/completions"
headers = {
    "Authorization": "Bearer hf_your_token",
    "Content-Type": "application/json",
}
payload = {
    "model": "Qwen/Qwen3-8B",
    "messages": [
        {"role": "user", "content": "Explain quantization in simple terms."}
    ],
    "max_tokens": 120,
    "temperature": 0.7,
}

r = requests.post(api_url, headers=headers, json=payload, timeout=60)
r.raise_for_status()
print(r.json()["choices"][0]["message"]["content"])
```

---

## 4. 양자화 (Quantization)

### 4.1 양자화란?

```
원본 (fp32)  →  양자화  →  저정밀도 (int8/int4)
 32 bits         ↓         8/4 bits
 큰 메모리       ↓         작은 메모리 (1/4 ~ 1/8)
```

| 양자화 방식 | 메모리 절약 | 정확도 손실 | 속도 |
|------------|-----------|-----------|------|
| fp16/bf16 | 2x | 거의 없음 | 빠름 |
| int8 | 4x | 약간 | 중간 |
| int4 (NF4) | 8x | 있음 | 느림 |
| GPTQ/AWQ | 8x | 적음 | 빠름 |

### 4.2 BitsAndBytes 8-bit 양자화

```python
from transformers import AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig
import torch

model_id = "meta-llama/Llama-3.2-3B-Instruct"

# 8-bit 양자화 설정
quantization_config = BitsAndBytesConfig(
    load_in_8bit=True,
)

tokenizer = AutoTokenizer.from_pretrained(model_id)
model = AutoModelForCausalLM.from_pretrained(
    model_id,
    quantization_config=quantization_config,
    device_map="auto",
)

# 메모리 확인
print(f"Memory footprint: {model.get_memory_footprint() / 1e9:.2f} GB")
# 약 3GB (원본 6GB 대비 ~50% 절약)
```

### 4.3 BitsAndBytes 4-bit 양자화 (NF4)

```python
from transformers import AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig
import torch

model_id = "Qwen/Qwen3-8B"

# 4-bit NF4 양자화 설정 (QLoRA용 최적)
quantization_config = BitsAndBytesConfig(
    load_in_4bit=True,
    bnb_4bit_quant_type="nf4",  # NF4 (정규분포 최적화)
    bnb_4bit_compute_dtype=torch.bfloat16,  # 계산은 bf16
    bnb_4bit_use_double_quant=True,  # 이중 양자화 (추가 메모리 절약)
)

tokenizer = AutoTokenizer.from_pretrained(model_id)
model = AutoModelForCausalLM.from_pretrained(
    model_id,
    quantization_config=quantization_config,
    device_map="auto",
)

# 메모리 확인
print(f"Memory footprint: {model.get_memory_footprint() / 1e9:.2f} GB")
# 약 4GB (원본 14GB 대비 ~70% 절약)
```

### 4.4 양자화 옵션 상세 설명

```python
from transformers import BitsAndBytesConfig
import torch

# 완전한 QLoRA 양자화 설정
quantization_config = BitsAndBytesConfig(
    # 기본 설정
    load_in_4bit=True,  # 4-bit 양자화 활성화
    
    # 양자화 타입
    bnb_4bit_quant_type="nf4",  
    # "nf4": Normal Float 4 (정규분포 가정, 추천)
    # "fp4": Float Point 4
    
    # 계산 데이터 타입
    bnb_4bit_compute_dtype=torch.bfloat16,
    # 양자화된 가중치를 사용해 계산할 때의 dtype
    # bf16: 더 빠른 계산, 약간 낮은 정밀도
    # fp16: 표준 반정밀도
    # fp32: 최고 정밀도, 느림
    
    # 이중 양자화
    bnb_4bit_use_double_quant=True,
    # 양자화 상수도 양자화 → 파라미터당 ~0.4bit 추가 절약
    
    # 8-bit 전용 옵션
    # llm_int8_threshold=6.0,  # 이상치 감지 임계값
    # llm_int8_skip_modules=["lm_head"],  # 양자화 제외 모듈
    # llm_int8_enable_fp32_cpu_offload=True,  # CPU 오프로드
)
```

### 4.5 메모리 사용량 비교

```python
import torch
from transformers import AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig

def load_model_with_memory_check(model_id: str, quant_type: str = None):
    """다양한 양자화 방식으로 모델 로드 및 메모리 확인"""
    
    torch.cuda.empty_cache()
    
    if quant_type == "4bit":
        config = BitsAndBytesConfig(
            load_in_4bit=True,
            bnb_4bit_quant_type="nf4",
            bnb_4bit_compute_dtype=torch.bfloat16,
        )
    elif quant_type == "8bit":
        config = BitsAndBytesConfig(load_in_8bit=True)
    else:
        config = None
    
    model = AutoModelForCausalLM.from_pretrained(
        model_id,
        quantization_config=config,
        device_map="auto",
        torch_dtype=torch.bfloat16 if config is None else None,
    )
    
    memory = model.get_memory_footprint() / 1e9
    print(f"{quant_type or 'bf16':>6}: {memory:.2f} GB")
    
    del model
    torch.cuda.empty_cache()
    return memory

# 비교 실행
model_id = "Qwen/Qwen3-8B"
print(f"\n{model_id} Memory Comparison:")
print("-" * 30)

# 주의: 순차적으로 실행해야 정확한 메모리 측정 가능
# load_model_with_memory_check(model_id, None)    # bf16
# load_model_with_memory_check(model_id, "8bit")  # int8
# load_model_with_memory_check(model_id, "4bit")  # int4
```

---

## 5. PEFT와 QLoRA 파인튜닝

### 5.1 LoRA 개념

```
┌─────────────────────────────────────────────────┐
│                    LoRA 개념                     │
│                                                 │
│  원본 가중치 W (frozen)  +  ΔW = A × B         │
│     [d × d]                 [d × r] × [r × d]  │
│                                                 │
│  r << d  →  학습 파라미터 대폭 감소             │
│  예: d=4096, r=16 → 99.6% 파라미터 절약        │
└─────────────────────────────────────────────────┘
```

### 5.2 LoraConfig 설정

```python
from peft import LoraConfig, TaskType

# LoRA 설정
lora_config = LoraConfig(
    # 핵심 파라미터
    r=16,  # LoRA rank (8~64 추천, 높을수록 표현력↑)
    lora_alpha=32,  # 스케일링 팩터 (보통 r의 2배)
    lora_dropout=0.05,  # 드롭아웃
    
    # 타겟 모듈
    target_modules="all-linear",  # 모든 Linear 레이어
    # 또는 명시적으로:
    # target_modules=["q_proj", "k_proj", "v_proj", "o_proj"],
    
    # 태스크 타입
    task_type=TaskType.CAUSAL_LM,
    
    # 추가 학습 모듈 (임베딩 포함 시)
    modules_to_save=["lm_head", "embed_tokens"],
    
    # 기타
    bias="none",  # bias 학습 여부 ("none", "all", "lora_only")
)
```

### 5.3 QLoRA 전체 파인튜닝 예제

```python
import torch
from datasets import load_dataset
from transformers import (
    AutoModelForCausalLM,
    AutoTokenizer,
    BitsAndBytesConfig,
)
from peft import LoraConfig, get_peft_model
from trl import SFTTrainer, SFTConfig

# 1. 모델 ID
model_id = "Qwen/Qwen3-4B"

# 2. 4-bit 양자화 설정
bnb_config = BitsAndBytesConfig(
    load_in_4bit=True,
    bnb_4bit_quant_type="nf4",
    bnb_4bit_compute_dtype=torch.bfloat16,
    bnb_4bit_use_double_quant=True,
)

# 3. 토크나이저 로드
tokenizer = AutoTokenizer.from_pretrained(model_id)
tokenizer.pad_token = tokenizer.eos_token

# 4. 모델 로드 (4-bit 양자화)
model = AutoModelForCausalLM.from_pretrained(
    model_id,
    quantization_config=bnb_config,
    device_map="auto",
)

# 5. LoRA 설정
lora_config = LoraConfig(
    r=16,
    lora_alpha=32,
    lora_dropout=0.05,
    target_modules="all-linear",
    task_type="CAUSAL_LM",
    modules_to_save=["lm_head", "embed_tokens"],
)

# 6. 데이터셋 로드
dataset = load_dataset("trl-lib/Capybara", split="train[:1000]")

# 7. SFTConfig 설정
training_args = SFTConfig(
    output_dir="./qwen-qlora-finetuned",
    max_seq_length=512,
    packing=True,  # 짧은 샘플들을 묶어서 효율적 학습
    
    # 학습 하이퍼파라미터
    num_train_epochs=1,
    per_device_train_batch_size=4,
    gradient_accumulation_steps=4,
    gradient_checkpointing=True,  # 메모리 절약
    
    # 옵티마이저
    learning_rate=2e-4,
    lr_scheduler_type="cosine",
    warmup_ratio=0.03,
    optim="adamw_torch_fused",
    
    # 정밀도
    bf16=True,
    
    # 로깅
    logging_steps=10,
    save_strategy="epoch",
    
    # Hub 업로드 (선택)
    # push_to_hub=True,
    # hub_model_id="username/model-name",
)

# 8. SFTTrainer 생성
trainer = SFTTrainer(
    model=model,
    args=training_args,
    train_dataset=dataset,
    peft_config=lora_config,
    processing_class=tokenizer,
)

# 9. 학습 실행
trainer.train()

# 10. 저장
trainer.save_model()
```

### 5.4 학습된 LoRA 어댑터 로드 및 추론

```python
from transformers import AutoModelForCausalLM, AutoTokenizer
from peft import PeftModel

# 베이스 모델 로드
base_model_id = "Qwen/Qwen3-4B"
adapter_path = "./qwen-qlora-finetuned"

tokenizer = AutoTokenizer.from_pretrained(base_model_id)
model = AutoModelForCausalLM.from_pretrained(
    base_model_id,
    device_map="auto",
    torch_dtype=torch.bfloat16,
)

# LoRA 어댑터 로드
model = PeftModel.from_pretrained(model, adapter_path)

# 추론
messages = [{"role": "user", "content": "Hello!"}]
prompt = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
inputs = tokenizer(prompt, return_tensors="pt").to(model.device)

with torch.no_grad():
    outputs = model.generate(**inputs, max_new_tokens=100)

print(tokenizer.decode(outputs[0], skip_special_tokens=True))
```

### 5.5 LoRA 어댑터 병합 (Merge)

```python
from transformers import AutoModelForCausalLM, AutoTokenizer
from peft import PeftModel

# 베이스 모델 로드 (fp16)
base_model = AutoModelForCausalLM.from_pretrained(
    "Qwen/Qwen3-4B",
    torch_dtype=torch.float16,
    device_map="auto",
)

# LoRA 어댑터 로드
model = PeftModel.from_pretrained(base_model, "./qwen-qlora-finetuned")

# 병합
merged_model = model.merge_and_unload()

# 병합된 모델 저장
merged_model.save_pretrained("./qwen-merged")
tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-4B")
tokenizer.save_pretrained("./qwen-merged")

# Hub에 업로드
# merged_model.push_to_hub("username/qwen-merged")
# tokenizer.push_to_hub("username/qwen-merged")
```

---

## 6. 추론 최적화

### 6.1 Flash Attention 2

```python
from transformers import AutoModelForCausalLM
import torch

model = AutoModelForCausalLM.from_pretrained(
    "Qwen/Qwen3-8B",
    torch_dtype=torch.bfloat16,
    device_map="auto",
    attn_implementation="flash_attention_2",  # Flash Attention 2 사용
)
```

### 6.2 SDPA (Scaled Dot-Product Attention)

```python
# PyTorch 2.0+ 기본 제공
model = AutoModelForCausalLM.from_pretrained(
    "Qwen/Qwen3-8B",
    torch_dtype=torch.bfloat16,
    device_map="auto",
    attn_implementation="sdpa",  # PyTorch native
)
```

### 6.3 추론 속도 비교 유틸리티

```python
import torch
import time
from transformers import AutoModelForCausalLM, AutoTokenizer

def benchmark_inference(model_id, quantization=None, attn_impl="sdpa", num_runs=5):
    """추론 속도 벤치마크"""
    
    # 모델 로드
    model = AutoModelForCausalLM.from_pretrained(
        model_id,
        torch_dtype=torch.bfloat16,
        device_map="auto",
        attn_implementation=attn_impl,
    )
    tokenizer = AutoTokenizer.from_pretrained(model_id)
    
    # 입력 준비
    prompt = "Write a short story about a robot learning to paint."
    inputs = tokenizer(prompt, return_tensors="pt").to(model.device)
    
    # 워밍업
    with torch.no_grad():
        _ = model.generate(**inputs, max_new_tokens=50)
    
    # 벤치마크
    times = []
    total_tokens = 0
    
    for _ in range(num_runs):
        torch.cuda.synchronize()
        start = time.time()
        
        with torch.no_grad():
            outputs = model.generate(**inputs, max_new_tokens=100)
        
        torch.cuda.synchronize()
        end = time.time()
        
        times.append(end - start)
        total_tokens += outputs.shape[1] - inputs["input_ids"].shape[1]
    
    avg_time = sum(times) / len(times)
    tokens_per_sec = total_tokens / sum(times)
    
    print(f"Model: {model_id}")
    print(f"Attention: {attn_impl}")
    print(f"Average time: {avg_time:.3f}s")
    print(f"Tokens/sec: {tokens_per_sec:.1f}")
    
    return tokens_per_sec
```

---

## 7. 실습 과제

### 과제 1: 모델 비교 분석

```python
"""
목표: 3개 이상의 Small LLM 비교 분석
요구사항:
1. 동일한 프롬프트로 응답 품질 비교
2. 메모리 사용량 비교
3. 추론 속도 비교
4. 결과를 표로 정리
"""

class ModelComparator:
    def __init__(self, model_ids: list[str]):
        self.model_ids = model_ids
        self.results = {}
    
    def compare_quality(self, prompt: str):
        """응답 품질 비교"""
        pass
    
    def compare_memory(self):
        """메모리 사용량 비교"""
        pass
    
    def compare_speed(self, num_runs: int = 5):
        """추론 속도 비교"""
        pass
    
    def generate_report(self) -> str:
        """비교 보고서 생성"""
        pass
```

### 과제 2: QLoRA 커스텀 파인튜닝

```python
"""
목표: 특정 도메인에 Small LLM 파인튜닝
요구사항:
1. 도메인 특화 데이터셋 준비 (예: 한국어 Q&A)
2. QLoRA 파인튜닝 실행
3. 파인튜닝 전후 성능 비교
4. 어댑터 Hub에 업로드
"""

def prepare_korean_dataset():
    """한국어 데이터셋 준비"""
    pass

def run_qlora_finetuning(dataset, model_id):
    """QLoRA 파인튜닝 실행"""
    pass

def evaluate_model(model, test_data):
    """모델 평가"""
    pass
```

---

## 8. 참고 링크

### 공식 문서
- [Transformers Quantization Guide](https://huggingface.co/docs/transformers/en/quantization/overview)
- [BitsAndBytes Documentation](https://huggingface.co/docs/transformers/en/quantization/bitsandbytes)
- [PEFT Documentation](https://huggingface.co/docs/peft)
- [TRL SFTTrainer](https://huggingface.co/docs/trl/en/sft_trainer)

### 모델 컬렉션
- [Qwen Collection](https://huggingface.co/collections/Qwen)
- [Llama Collection](https://huggingface.co/collections/meta-llama)
- [Gemma Collection](https://huggingface.co/collections/google/gemma-3-release)
- [Phi Collection](https://huggingface.co/collections/microsoft/phi-4)

### 유용한 블로그/튜토리얼
- [Making LLMs accessible with 4-bit quantization](https://huggingface.co/blog/4bit-transformers-bitsandbytes)
- [Fine-tune LLMs in 2025](https://www.philschmid.de/fine-tune-llms-in-2025)
- [QLoRA Paper](https://arxiv.org/abs/2305.14314)

### Leaderboard
- [Open LLM Leaderboard](https://huggingface.co/spaces/open-llm-leaderboard/open_llm_leaderboard)

---

## 9. 핵심 요약

### Small LLM 선택 플로우차트

```
시작
  │
  ├─ 메모리 < 8GB?
  │    ├─ Yes → SmolLM, Qwen-0.5B/1.5B, Llama-1B
  │    └─ No ↓
  │
  ├─ 용도?
  │    ├─ 코딩 → Qwen-Coder, DeepSeek-Coder, CodeLlama
  │    ├─ 수학/추론 → Phi-3/4, Qwen-Math, DeepSeek-R1
  │    ├─ 다국어 → Qwen3, Gemma 3
  │    └─ 범용 → Llama 3.2, Qwen3, Gemma 3, Kimi-K2.5
  │
  ├─ 양자화 필요?
  │    ├─ 메모리 부족 → 4-bit (NF4)
  │    ├─ 균형 → 8-bit
  │    └─ 최고 품질 → bf16
  │
  └─ 파인튜닝 필요?
       ├─ Yes → QLoRA + SFTTrainer
       └─ No → 바로 사용
```

### 메모리 요구사항 가이드

| 모델 크기 | fp16 | 8-bit | 4-bit |
|----------|------|-------|-------|
| 1B | ~2 GB | ~1 GB | ~0.5 GB |
| 3B | ~6 GB | ~3 GB | ~1.5 GB |
| 7B | ~14 GB | ~7 GB | ~4 GB |
| 13B | ~26 GB | ~13 GB | ~7 GB |

> 💡 **Tip**: QLoRA 학습 시 4-bit 모델 + LoRA 어댑터로 7B 모델도 16GB GPU에서 학습 가능!
